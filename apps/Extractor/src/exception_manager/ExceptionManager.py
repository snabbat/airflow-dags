...
class ExceptionManager:
    """Gestionnaire central des exceptions. Charge la configuration et orchestre la chaine de traitement."""
    ...
    def __init__(self, config_path, file_logger, print_logger):
        ...
        # Configuration automatique des handlers a la creation
        self.setup_handlers()

    def _load_config(self, config_path):
        """Charge la configuration des exceptions depuis un fichier JSON."""
        try:
    ...
    def setup_handlers(self):
        """Configure les gestionnaires d'exceptions selon la configuration chargee."""
    ...
    def handle(self, exception, context):
        """Passe l'exception dans la chaine de gestionnaires puis enregistre le resultat."""
        if self.chain:
            error_info = self.chain.handle_exception(exception, context)
            if isinstance(error_info, dict):
                # Formatage et ecriture dans le fichier de log
                ...
                # Formatage et affichage sur la console
